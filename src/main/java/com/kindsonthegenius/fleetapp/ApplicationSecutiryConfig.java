...
public class ApplicationSecutiryConfig extends WebSecurityConfigurerAdapter {
	...
	protected void configure(HttpSecurity http) throws Exception {
		...
		.authorizeRequests()
				/*
				 * permit resources used to design login page to be access without
				 * authentication thus css, fonts and others
				 */
		.antMatchers("/login", "/resources/**", "/css/**", "/fonts/**", "/img/**").permitAll()
		.anyRequest().authenticated()
		.and()
		.formLogin()/* indicate we are using form login */
		.loginPage("/login").permitAll() /* permits the login page thus the path */
		.and()
		.logout().invalidateHttpSession(true)/* invalidate session when logout */
		.clearAuthentication(true)/* clears the authentication */
		.logoutRequestMatcher(new AntPathRequestMatcher("/logout")) /* then logout */
